...
public class TestFifoQueue {
	...
	private Iterator<String> stringItr;
	...
	/**
	 * Test that iterator.hasNext() returns false and iterator.next() throws
	 * a NoSuchElementException when the Queue is empty
	 */
	@Test( expected =  NoSuchElementException.class )
	public final void testIteratorEmptyQueue() {
		intItr = myIntQueue.iterator();
		stringItr = myStringQueue.iterator();
		assertFalse("hasNext called on empty (int) queue: ", intItr.hasNext());
		assertFalse("hasNext called on empty (String) queue: ", stringItr.hasNext());
	...
	/*
	 * Test that iterator.remove() throws a UnsupportedOperationException
	 */
	...
	@Test
	public final void testIteratorReturns() {
		for (int i = 0; i < 10; i++) {
			myIntQueue.add(i);
		}
		...
		int k = 0;
		for (int i = 0; i < 10; i++) {
			int x = (int) intItr.next();
			assertEquals("Iterator returns wrong element: ", k, x);
			System.out.println(x);
			if (i != 9) {
				assertTrue("Should return true on hasNext(): ", intItr.hasNext());
			}
			k++;
		}
		assertFalse("Iterator should have gone through all elements by now: ", intItr.hasNext());
	}
